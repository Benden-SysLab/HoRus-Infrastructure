...
## Stage 3: Service Databases, Access Hardening, HashiCorp Vault Platform, GitGuardian, and Gitea Integration

This Stage handles the complete deployment of platform service storage backends, credential management, security baselines, and enterprise secrets management via HashiCorp Vault, Gitea Git platform, and GitGuardian security control plane.
...
### Stage 3.2 - 3.5: HashiCorp Vault Platform Integration
...
### Stage 3.3: GitGuardian Security Platform Deployment
Deploys and configures a highly hardened **GitGuardian Security Platform** serving as a central Security Control Plane component.
- **Zero-Trust Hardened Service**: Configured with a dedicated system user (`gitguardian:nologin`) and deep systemd sandboxing (including `ProtectSystem=strict`, `PrivateDevices=true`, `ProtectKernelTunables=true`, `ProtectKernelModules=true`, `ProtectControlGroups=true`, `RestrictSUIDSGID=true`, and `CapabilityBoundingSet=` to completely drop Linux capabilities).
- **Security Policy Enforcement**: Structured via `/etc/gitguardian/policy.yml` defining allowed secret types, severity mapping, path patterns, and specific secret ignore lists.
- **Gitea Global Webhook Integration**: Leverages Gitea system webhooks to automatically monitor all repository push events across the platform with no manual configuration required.
- **Prometheus Monitoring & Alertmanager Rules**: Exposes native metrics (`gitguardian_secrets_found_total`) and deploys alerting rules (`CriticalSecretLeak`) to trigger immediate incident management if secrets are leaked.
- **Software Bill of Materials (SBOM)**: Automatically generates and validates a CycloneDX 1.4 compliant SBOM (`sbom.json`) of the running environment for vulnerability scanning and license tracking.
- **SRE Smoke Tests & Validation**: Features a rigorous automated end-to-end smoke test verifying the entire workflow: temporary private repo creation -> committing real test AWS credentials -> pushing -> webhook triggering -> GitGuardian scanning -> PostgreSQL event logging -> finding API validation -> clean-up. Includes controlled degraded mode with local encrypted credential cache in case of Vault unavailability.

### Stage 3.4: Gitea Production Deployment
Deploys a robust, self-hosted, enterprise-grade Git service Gitea, fully integrated into the HoRus security plane.
- **Database & Secrets Isolation**: Backed by PostgreSQL with SCRAM-SHA-256 TCP/IP communication and secured using dynamic Vault-based AppRole authentication, avoiding static inline plaintext passwords.
- **Custom Systemd Orchestration**: Hardened systemd execution context restricting file system permissions and resource bounds.
- **Automation Pipeline Ready**: Serves as the source code repository hub, integrated with global security webhooks for continuous scanning, ready for downstream OIDC authentication and CI/CD triggers.
...
- [x] **Stage 3.2**: HashiCorp Vault Platform Integration (Deployment, Policies & AppRoles, Secrets Bootstrap, SRE Validation & Self-Healing Disaster Recovery).
- [x] **Stage 3.3**: GitGuardian Security Platform Deployment (Zero Trust Secrets Scanning, Security Policy, SRE Validation, Prometheus Rules, and SBOM).
- [x] **Stage 3.4**: Gitea Production Deployment (PostgreSQL SCRAM auth, Vault AppRole orchestration, systemd sandbox hardening).
- [ ] **Stage 4**: Identity & Control Plane Deployment (Authentik IAM, Jenkins, Harbor, OIDC, Monitoring).
